#!/usr/bin/env node
/**
 * Применить или откатить перенос 30 SKU Wave 1b по manifest.
 *
 *   node ops/seo/apply-seo-wave1b-reclass.mjs --apply
 *   node ops/seo/apply-seo-wave1b-reclass.mjs --rollback
 *   node ops/seo/apply-seo-wave1b-reclass.mjs --dry-run
 *
 * Backup (не в git): server/data/backups/seo-wave1b-20260828T211120Z/
 */
...
const MANIFEST_PATH = path.join(__dirname, "seo-wave1b-reclass-manifest.json");
const DB_PATH =
...
if (!mode) {
  console.error(
    "Usage: node ops/seo/apply-seo-wave1b-reclass.mjs --apply|--rollback|--dry-run"
  );
  process.exit(2);
}
...
if (moves.length !== 30) {
  console.error(`Expected 30 moves in manifest, got ${moves.length}`);
  process.exit(1);
}
