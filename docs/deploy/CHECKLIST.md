...
5. Подготовить inbound ключ(и) только для контуров в `ONEC_ALLOWED_DATABASES` (≥ 24, разные при нескольких); для VLAVKA-only достаточно `ONEC_VLAVKA_EXCHANGE_API_KEY`; `ONEC_API_KEY` только для outbound при необходимости.
