...
5. Подготовить отдельные сложные inbound ключи `ONEC_TEST_EXCHANGE_API_KEY` и `ONEC_VLAVKA_EXCHANGE_API_KEY` (≥ 24, разные); `ONEC_API_KEY` только для outbound при необходимости.
