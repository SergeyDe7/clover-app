...
db.exec(`
  ...
  CREATE TABLE IF NOT EXISTS product_translations (
    ...
    language_code TEXT NOT NULL CHECK(language_code IN ('en', 'uz', 'ky', 'tg', 'zh-CN', 'ar')),
    field_key TEXT NOT NULL CHECK(field_key IN ('name', 'description', 'composition', 'characteristics')),
    auto_value TEXT NOT NULL DEFAULT '',
    auto_source_hash TEXT NOT NULL DEFAULT '',
    auto_run_id TEXT NOT NULL DEFAULT '',
    auto_generated_at TEXT NOT NULL DEFAULT '',
    manual_value TEXT NOT NULL DEFAULT '',
    manual_source_hash TEXT NOT NULL DEFAULT '',
    updated_at TEXT NOT NULL,
    updated_by TEXT NOT NULL DEFAULT '',
    CHECK(
      (length(trim(auto_value)) = 0 AND length(trim(auto_source_hash)) = 0)
      OR (length(trim(auto_value)) > 0 AND length(trim(auto_source_hash)) = 64)
    ),
    CHECK(
      (length(trim(manual_value)) = 0 AND length(trim(manual_source_hash)) = 0)
      OR (length(trim(manual_value)) > 0 AND length(trim(manual_source_hash)) = 64)
    ),
    PRIMARY KEY(product_id, language_code, field_key)
  ) STRICT;

  CREATE INDEX IF NOT EXISTS idx_product_translations_language
    ON product_translations(language_code, field_key);
  ...
  CREATE TABLE IF NOT EXISTS translation_glossary (
    ...
    context TEXT NOT NULL DEFAULT '' CHECK(context IN ('', 'product.name', 'product.description', 'product.composition', 'product.characteristics')),
  ...
  CREATE INDEX IF NOT EXISTS idx_translation_glossary_language
    ON translation_glossary(language_code, normalized_source);
`);
...
export function listProductTranslationRows() {
  return db
    .prepare(
...
export function listProductTranslationRowsForProduct(productId) {
  return db
    .prepare(
      `SELECT product_id AS productId, language_code AS languageCode, field_key AS fieldKey,
              auto_value AS autoValue, auto_source_hash AS autoSourceHash,
    ...
    db
      .prepare(
        `SELECT product_id AS productId, language_code AS languageCode, field_key AS fieldKey,
                ...
                updated_at AS updatedAt, updated_by AS updatedBy
         FROM product_translations
         WHERE product_id = ? AND language_code = ? AND field_key = ?`
      )
      .get(String(productId || ""), String(languageCode || ""), String(fieldKey || "")) || null
  );
}

export function upsertProductTranslationRow(row) {
  db.prepare(
    `INSERT INTO product_translations(
       product_id, language_code, field_key, auto_value, auto_source_hash,
       auto_run_id, auto_generated_at, manual_value, manual_source_hash, updated_at, updated_by
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(product_id, language_code, field_key) DO UPDATE SET
       auto_value = excluded.auto_value,
       auto_source_hash = excluded.auto_source_hash,
       auto_run_id = excluded.auto_run_id,
       auto_generated_at = excluded.auto_generated_at,
       manual_value = excluded.manual_value,
       manual_source_hash = excluded.manual_source_hash,
       updated_at = excluded.updated_at,
       updated_by = excluded.updated_by`
  ).run(
    row.productId,
    ...
    row.autoGeneratedAt || "",
    row.manualValue || "",
    row.manualSourceHash || "",
    row.updatedAt,
    row.updatedBy || ""
  );
}

export function deleteProductTranslationRows(productId) {
  return db
...
export function listGlossaryRows() {
  return db
    .prepare(
      `SELECT id, source_ru AS sourceRu, normalized_source AS normalizedSource,
              language_code AS languageCode, target_value AS targetValue,
              context, normalized_context AS normalizedContext, protected,
              created_at AS createdAt, updated_at AS updatedAt, updated_by AS updatedBy
       FROM translation_glossary
       ORDER BY length(source_ru) DESC, source_ru, language_code`
    )
    .all();
}

export function getGlossaryRow(id) {
  return (
    db
      .prepare(
        `SELECT id, source_ru AS sourceRu, normalized_source AS normalizedSource,
...
export function findGlossaryByIdentity(normalizedSource, languageCode, normalizedContext) {
  return (
    db
      .prepare(
        `SELECT id, source_ru AS sourceRu, normalized_source AS normalizedSource,
                ...
                created_at AS createdAt, updated_at AS updatedAt, updated_by AS updatedBy
         FROM translation_glossary
         WHERE normalized_source = ? AND language_code = ? AND normalized_context = ?`
      )
      .get(
...
export function insertGlossaryRow(row) {
  db.prepare(
    `INSERT INTO translation_glossary(
       id, source_ru, normalized_source, language_code, target_value, context,
       normalized_context, protected, created_at, updated_at, updated_by
     ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
  ).run(
    row.id,
    row.sourceRu,
...
export function updateGlossaryRow(row) {
  db.prepare(
    `UPDATE translation_glossary
     SET source_ru = ?, normalized_source = ?, language_code = ?, target_value = ?,
         context = ?, normalized_context = ?, protected = ?, updated_at = ?, updated_by = ?
     WHERE id = ?`
  ).run(
    row.sourceRu,
    row.normalizedSource,
...
export function deleteGlossaryRow(id) {
  return db.prepare("DELETE FROM translation_glossary WHERE id = ?").run(String(id || "")).changes;
}
...
export function exportDatabaseSnapshot() {
  return {
    ...
    productTranslations: db.prepare(`
      SELECT product_id, language_code, field_key, auto_value, auto_source_hash,
             auto_run_id, auto_generated_at,
             manual_value, manual_source_hash, updated_at, updated_by
      FROM product_translations
      ORDER BY product_id, language_code, field_key
    `).all(),
    translationGlossary: db.prepare(`
      SELECT id, source_ru, normalized_source, language_code, target_value, context,
             normalized_context, protected, created_at, updated_at, updated_by
      FROM translation_glossary
      ORDER BY created_at
    `).all(),
...
export function importDatabaseSnapshot(snapshot) {
  ...
  try {
    ...
    db.exec("DELETE FROM translation_glossary");
    ...
    const insertProductTranslation = db.prepare(`
      INSERT INTO product_translations(
        product_id, language_code, field_key, auto_value, auto_source_hash,
        auto_run_id, auto_generated_at, manual_value, manual_source_hash, updated_at, updated_by
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    for (const row of Array.isArray(snapshot.productTranslations) ? snapshot.productTranslations : []) {
      insertProductTranslation.run(
        String(row.product_id || row.productId || ""),
        String(row.language_code || row.languageCode || ""),
        String(row.field_key || row.fieldKey || ""),
        String(row.auto_value || row.autoValue || ""),
        String(row.auto_source_hash || row.autoSourceHash || ""),
        String(row.auto_run_id || row.autoRunId || ""),
        String(row.auto_generated_at || row.autoGeneratedAt || ""),
        String(row.manual_value || row.manualValue || ""),
        String(row.manual_source_hash || row.manualSourceHash || ""),
        String(row.updated_at || row.updatedAt || now()),
        String(row.updated_by || row.updatedBy || "")
      );
    }
    ...
    `);
    for (const row of Array.isArray(snapshot.translationGlossary) ? snapshot.translationGlossary : []) {
      insertGlossary.run(
        String(row.id || randomUUID()),
        String(row.source_ru || row.sourceRu || ""),
        String(row.normalized_source || row.normalizedSource || ""),
        String(row.language_code || row.languageCode || ""),
        String(row.target_value || row.targetValue || ""),
        String(row.context || ""),
        String(row.normalized_context || row.normalizedContext || ""),
        Number(row.protected) ? 1 : 0,
        String(row.created_at || row.createdAt || now()),
        String(row.updated_at || row.updatedAt || now()),
        String(row.updated_by || row.updatedBy || "")
      );
    }
